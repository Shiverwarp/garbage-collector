...
function dessertIslandWorthIt(): boolean {
  // guesstimating value of giant crab at 5*VOA
  if (garboValue($item`cocoa of youth`) > 5 * get("valueOfAdventure")) {
    return true;
  }
  return false;
}
  ...
  tasks: [
    {
      ...
      completed: () => questStep("_questPirateRealm") > 5,
      prepare: () => {
        meatMood("Greg", false, targetMeat()).execute(copyTargetCount());
        potionSetup(false);
        checkAndFixOvercapStats();
      },
